...
func (c *Client) Read(cxt context.Context) {
	defer func(cxt context.Context) {
		...
		log.Debugf("client [%s] disconnect", c.Id)
		if err := c.Socket.Close(); err != nil {
			log.Debugf("client [%s] disconnect err: %s", c.Id, err)
		}
	}(cxt)
	...
	for {
		...
		log.Debugf("client [%s] receive message: %s", c.Id, string(message))
		c.Message <- message
	}
}
...
func (c *Client) Write(cxt context.Context) {
	...
	for {
		...
		case message, ok := <-c.Message:
			...
			log.Debugf("client [%s] write message: %s", c.Id, string(message))
			err := c.Socket.WriteMessage(websocket.TextMessage, message)
			if err != nil {
				log.Debugf("client [%s] writemessage err: %s", c.Id, err)
			}
...
func (manager *Manager) Start() {
	for {
		...
		case client := <-manager.Register:
			log.Debugf("client [%s] connect", client.Id)
			log.Debugf("register client [%s] to group [%s]", client.Id, client.Group)
		...
		case client := <-manager.UnRegister:
			log.Debugf("unregister client [%s] from group [%s]", client.Id, client.Group)
			manager.Lock.Lock()
			if mGroup, ok := manager.Group[client.Group]; ok {
				if mClient, ok := mGroup[client.Id]; ok {
					...
					manager.clientCount -= 1
					if len(mGroup) == 0 {
						//log.Debugf("delete empty group [%s]", client.Group)
