...
import { getMenuRoleList } from '/@/api/sys/menu';
...
import feat from '/@/router/routes/modules/demo/feat';
  ...
  actions: {
    ...
    async buildRoutesAction(): Promise<AppRouteRecordRaw[]> {
      ...
      const routeFilter = (route: AppRouteRecordRaw) => {
        ...
        // 进行角色权限判断, SUPER override everything
        return roleList.includes(RoleEnum.SUPER) || roleList.some((role) => roles.includes(role));
      };
      ...
      switch (permissionMode) {
        ...
        case PermissionModeEnum.BACK:
          ...
          try {
            const menuList = await getMenuRoleList();
            routeList = transformMenuToRoute(menuList);
          } catch (error) {
            console.error(error);
          }
