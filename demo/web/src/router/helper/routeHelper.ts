...
// Turn background menus into raw routing objects
export function transformMenuToRoute(menuList: RouteItem[]): AppRouteRecordRaw[] {
  return menuList.map((m) => {
    return {
      ...
      meta: {
        ...
        orderNo: m.sort,
        title: m.title,
        ignoreKeepAlive: m.noCache,
        icon: m.iconAntd,
      },
      children: m.children ? transformMenuToRoute(m.children) : undefined,
    };
  });
}
