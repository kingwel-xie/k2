<script lang="tsx">
  ...
  import { formatItem } from '/@/components/Description/src/useDescription';
        ...
        return unref(schema)
          .map((item) => {
            ...
            const getContent = () => {
              ...
              // construct render function if there is no render specified
              if (!render && format) {
                item.render = (val, data) => {
                  return formatItem(val, data, format);
                };
              }
              const getField = get(_data, field);
              if (getField && !toRefs(_data).hasOwnProperty(field)) {
                return isFunction(item.render) ? item.render('', _data) : '';
              }
              return isFunction(item.render) ? item.render(getField, _data) : getField ?? '';
            };
