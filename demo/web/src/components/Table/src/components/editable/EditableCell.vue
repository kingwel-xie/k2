<script lang="tsx">
  ...
  import { formatToDictLabel, formatValue } from '/@/utils/formatUtil';
    ...
    setup(props) {
      ...
      const getDisable = computed(() => {
        ...
        if (isFunction(editDynamicDisabled)) {
          const { record } = props;
          disabled = (record && editDynamicDisabled(record)) || false;
        }
        return disabled;
      });
      const getValues = computed(() => {
        const { editValueMap, format } = props.column;
        ...
        if (!component.includes('Select') && !component.includes('Radio')) {
          // kingwel: if there is a format string
        ...
        // kingwel: DictSelect/Radio...
        if (unref(getComponentProps)?.dictName) {
          return formatToDictLabel(value, unref(getComponentProps)?.dictName);
        }
          ...
          {this.isEdit && (
            <Spin spinning={this.spinning}>
              <div class={`${this.prefixCls}__wrapper`} v-click-outside={this.onClickOutside}>
                <CellComponent
                  {...omit(this.getComponentProps, ['onChange'])}
                ...
                {!this.getRowEditable && (
                  <div class={`${this.prefixCls}__action`}>
                    ...
                    />
                    <CloseOutlined class={`${this.prefixCls}__icon `} onClick={this.handleCancelClick} />
                  </div>
                )}
              </div>
            </Spin>
