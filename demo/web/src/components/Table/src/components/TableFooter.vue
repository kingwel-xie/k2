  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const getColumns = computed(() => {
        ...
        const tableColumns: BasicColumn[] = cloneDeep(table.getColumns());
        const index = tableColumns.findIndex((item) => item.flag === INDEX_COLUMN_FLAG);
        ...
        if (dataSource.length > 0) {
          Object.keys(dataSource[0]).forEach((key) => {
            // skip row key
            if (props.rowKey === key) return;
            // try to locate the key in tableColumns
            let col = tableColumns.find((col) => col.dataIndex === key);
            if (!col) {
              col = {
                width: 80,
                title: 'key',
            ...
            columns.push(omit(col, ['customRender', 'customCell']));
          });
        }
