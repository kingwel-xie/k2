...
// @Tags 菜单
// @Success 200 {object} response.Response "{"code": 200, "data": [...]}"
// @Router /api/v1/menu-role [get]
// @Security Bearer
func (e SysMenu) GetMenuRole(c *gin.Context) {
	s := new(service.SysMenu)
	_ = e.MakeContext(c).MakeService(&s.Service)

	result, err := s.GetMenuRole(s.Identity.RoleKey)
	if err != nil {
		e.Error(500, err, "查询失败")
		return
	}
...
// @Success 200 {object} response.Response "{"code": 200, "data": [...]}"
// @Router /api/v1/roleMenuTreeSelect/{roleId} [get]
// @Security Bearer
